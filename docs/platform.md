...
The central management plane for an Organisation. **Org-scoped**: shared across all VPC environments — editing Keycloak realms or OpenBao PKI in "dev" propagates to "staging" and "prod" (same instance).

| Service | Purpose | User-Editable |
|---------|---------|---------------|
| **Keycloak** | Identity & Access Management (SSO, MFA, Federation) | Sub-graph only (realms, clients) |
| **OpenBao** | Secrets management, PKI (root/intermediate CAs, certificate issuance) | Sub-graph only (PKI engines, auth backends) |
| **K8s Control Plane** | Manages Kubernetes clusters across VPC environments | No (system-managed) |

**Bastion infrastructure** (system-managed, not user-editable):
- Dedicated databases (e.g., PostgreSQL for Keycloak)
- Network chain (Internet → Gateway → VirtualService → K8sService)

In the console graph editor, bastion nodes appear **in the environment graph** with `scope: 'org'` and are visually distinguished (dashed border, "ORG" badge, locked). Changes to bastion nodes are saved to the org-level API (`/api/organizations/:orgId/bastion-graph`), separate from the env-level graph.

### 5. VPC Environments

Isolated network environments connected to the Bastion via VPC routing. Each environment has its own **env-scoped** graph for user workloads.
