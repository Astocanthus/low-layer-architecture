...
│
├── low-layer-console                # Web console - authenticated (Angular 21)
│
...
│   │   ├── env/               # lowlayer env create/list/delete
...
lowlayer env create <org> <name>         # Create environment
lowlayer node provision <env> --count 3
lowlayer cluster kubeconfig <env>        # Get kubeconfig
lowlayer catalog list                    # List available services
lowlayer catalog deploy postgres <env>   # Deploy service
```
...
**Language**: TypeScript | **Framework**: Angular 21 (standalone, flat structure) | **URL**: https://cloud.low-layer.com

The console is the largest and most complex repository. Its architecture is documented separately:

- **[Console Architecture](console/architecture.md)** — Core module (auth, HTTP, ApiService, BaseRepository), shared module (Zod schemas), features (dashboard, organizations, catalog), MSW mock system, environment config, path aliases
- **[Graph Editor Architecture](console/graph-editor.md)** — Rete.js v2 graph editor: topology model, typed socket/pin system, connection validation, relation editor, auto-chain, sub-graph editor
...
│       ├── environment.go     # lowlayer_environment
...
# Environment
resource "lowlayer_environment" "prod_main" {
...
resource "lowlayer_node" "workers" {
  environment_id = lowlayer_environment.prod_main.id
  count          = 3
  flavor         = "m1.large"
  role           = "worker"
}
